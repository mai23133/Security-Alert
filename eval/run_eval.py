...
DEFAULT_ALLOWLIST = PROJECT_ROOT / "data" / "processed" / "technique_ids.json"
...
def build_records(dataset: dict[str, Any], predictions: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    if any(not isinstance(item, dict) for item in alerts + saved_predictions):
        raise ValueError("dataset and predictions must contain objects")
    dataset_alert_ids = [item.get("alert_id") for item in alerts]
    ...
    # Gold fields always belong to the dataset, never to model output.
    fields = ("inferred_techniques", "candidates", "needs_human_review")
    return [
        {**alert, **{key: prediction_by_id[alert["alert_id"]][key] for key in fields}}
        for alert in alerts
    ]
...
def validate_dataset(dataset: dict[str, Any], allowlist: set[str]) -> None:
    ...
    for item in alerts:
        ...
        if not isinstance(narrative, str) or not narrative.strip() or len(narrative) > 20_000:
            raise ValueError(f"dataset alert {alert_id} narrative must be nonempty text up to 20000 characters")
...
def validate_predictions(
    ...
    for item in saved_predictions:
        alert_id = item["alert_id"]
        if {"gold_technique_ids", "narrative", "category"} & item.keys():
            raise ValueError("predictions must not contain dataset-owned fields")
...
def main() -> int:
    ...
    print(rendered)
    return int(args.require_quality_gates and not report["numeric_gates_passed"])
