...
| 2 | 27 ก.ค.–2 ส.ค. 2026 | P1 | เลือกและบันทึก embedding backend กับรูปแบบ index ยืนยันการใช้ schema `tactic: str` สร้าง `embedder.py` และ index พร้อม metadata สร้าง `retriever.py` ที่กรอง tactic และกำหนด top-k ได้ เพิ่ม retrieval tests และชุดประเมิน Recall@k เบื้องต้น | Retriever คืน top-k ที่ทำซ้ำได้จาก pinned subset เท่านั้น มี metadata ของ tactic, platform, source และมีผล Recall@1, Recall@3, Recall@5 baseline |
